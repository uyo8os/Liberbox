...
module.exports = function initTunManager(context) {
  ...
  function getCoreService() {
    ...
    if (!coreServiceModule) {
      try {
        ...
        console.warn(
          "[TunManager] Failed to load core-service module:",
          e.message,
        );
      }
    }
  ...
  function getServiceIpc() {
    ...
    if (!serviceIpcModule) {
      try {
        ...
        console.warn(
          "[TunManager] Failed to load service-ipc module:",
          e.message,
        );
      }
    }
  ...
  function isSameKernelBinary(sourcePath, targetPath) {
    try {
      ...
      console.warn(
        "[TunManager] Failed to compare kernel binaries:",
        e?.message || e,
      );
      return false;
    }
  ...
  function getKernelPath() {
    if (isMac) {
      ...
      if (fs.existsSync(systemPath)) {
        ...
        if (st.uid === 0 && st.isSetuid) {
          console.log(
            "[TunManager] Using authorized system kernel:",
            systemPath,
          );
          return systemPath;
        }
    ...
    try {
      ...
      console.error(
        "[TunManager] Failed to get kernel path from mihomoService:",
        e,
      );
    }
    ...
    try {
      if (typeof context.getKernelExecutablePath === "function") {
        ...
        if (kernelPath && fs.existsSync(kernelPath)) {
          console.log(
            "[TunManager] Using kernel path from context:",
            kernelPath,
          );
          return kernelPath;
        }
  ...
  function getSourceKernelPath() {
    ...
    try {
      ...
      if (
        ...
        path.resolve(managedPath) !== path.resolve(systemPath)
      ) {
        return managedPath;
      }
    } catch (e) {
      console.warn(
        "[TunManager] Failed to get source kernel from coreManager:",
        e?.message || e,
      );
    }
    ...
    try {
      if (typeof context.getKernelExecutablePath === "function") {
        ...
        if (
          ...
          path.resolve(kernelPath) !== path.resolve(systemPath)
        ) {
          return kernelPath;
        }
      }
    ...
    try {
      if (typeof context.resolveDefaultKernelPath === "function") {
        ...
        if (
          ...
          path.resolve(defaultPath) !== path.resolve(systemPath)
        ) {
          return defaultPath;
        }
      }
  ...
  function isTunActive() {
    try {
      if (isMac) {
        const out = execSync("/sbin/ifconfig -l", {
          stdio: ["ignore", "pipe", "ignore"],
        }).toString();
        return /\butun\d+\b/.test(out);
      }
      if (isLinux) {
        const out = execSync("ip link show", {
          stdio: ["ignore", "pipe", "ignore"],
        }).toString();
  ...
  function getActiveNetworkService() {
    ...
    try {
      ...
      for (const service of serviceLines) {
        try {
          const hw = execSync(
            `networksetup -listallhardwareports | grep -A 1 "${service}"`,
            { encoding: "utf8" },
          );
  ...
  async function setSystemDns(dns) {
    ...
    // 备份当前 DNS
    try {
      const current = execSync(`networksetup -getdnsservers "${service}"`, {
        encoding: "utf8",
      }).trim();
    ...
    try {
      ...
      const useService =
        serviceManager && (await serviceManager.isServiceRunning());

      if (useService) {
        const result = await serviceManager.setSystemDns(service, dns);
        if (result.success) {
          console.log(
            "[TunManager] System DNS set via service (no password needed)",
          );
          return { success: true, via: "service" };
        }
        console.warn(
          "[TunManager] Service setDns failed, will rely on DNS hijacking",
        );
        return {
          success: false,
      ...
      console.log(
        "[TunManager] Service not running, relying on TUN DNS hijacking (no password needed)",
      );
    ...
    try {
      ...
      if (useService) {
        ...
        if (result.success) {
          ...
          console.log(
            "[TunManager] System DNS restored via service (no password needed)",
          );
          return { success: true, via: "service" };
        }
      ...
      console.log(
        "[TunManager] Service not running, DNS will restore naturally (no password needed)",
      );
      // 删除备份文件，避免下次误恢复
      try {
        fs.unlinkSync(dnsBackupFile);
      ...
      return {
        success: false,
        reason: "no_service",
        note: "dns_will_restore_naturally",
      };
    } catch (e) {
  ...
  async function probeAuthorization(kernelPath) {
    ...
    if (false && isMac) {
      // legacy block disabled; robust mac flow handled above
      ...
      try {
        const cap = execSync(`getcap "${kernelPath}" || true`, {
          stdio: ["ignore", "pipe", "ignore"],
        }).toString();
        if (!/cap_net_admin/i.test(cap))
          result.issues.push("cap_net_admin_missing");
      } catch {
        result.issues.push("cap_check_failed");
      }
    }
    ...
    try {
      fs.writeFileSync(probeConfig, yaml.dump(conf), "utf8");
    } catch {}
  ...
  async function grantPermissions(opts = {}) {
    ...
    if (isMac) {
      try {
        ...
        const samePath =
          path.resolve(sourceKernelPath) === path.resolve(systemPath);
        const needsCopy =
        ...
        if (existingProbe.ok && !needsCopy) {
          console.log(
            "[TunManager] System kernel already authorized and up-to-date",
          );
        ...
        if (useService) {
          ...
          if (needsCopy) {
            ...
            const tmpPath = path.join(
              os.tmpdir(),
              "flycast-mihomo-" + crypto.randomBytes(8).toString("hex"),
            );
            try {
          ...
          console.warn(
            "[TunManager] Service authorization failed, falling back to osascript",
          );
        }

        console.log(
          "[TunManager] Using osascript to copy and authorize kernel",
        );
        ...
        return {
          success: false,
          error: getUserFriendlyError(e, "authorization"),
        };
      }
    }
    ...
    if (isLinux) {
      try {
        ...
        if (useService) {
          console.log(
            "[TunManager] Using service mode for authorization on Linux",
          );
          ...
          console.warn(
            "[TunManager] Service authorization failed, falling back to pkexec",
          );
        }
        ...
        try {
          execFileSync(
            "pkexec",
            ["setcap", "cap_net_admin,cap_net_bind_service=+eip", kernelPath],
            { stdio: "ignore" },
          );
        } catch {
          try {
            execFileSync("pkexec", ["chown", "root:root", kernelPath], {
              stdio: "ignore",
            });
            execFileSync("pkexec", ["chmod", "+sx", kernelPath], {
              stdio: "ignore",
            });
          } catch (e) {
            return {
        ...
        return probe.ok
          ? { success: true }
          : { success: false, error: "授权验证失败，请重试" };
      } catch (e) {
        console.error("[TunManager] Grant permissions failed:", e);
        return {
          success: false,
    ...
    if (isWindows) {
      try {
        ...
        if (elevationMode === "service") {
          ...
          if (!coreService) {
            console.warn(
              "[TunManager] Core service module not available, falling back to task mode",
            );
          ...
          const kernelPath = getKernelPath();
          const installResult = await coreService.install({
            corePath: kernelPath,
          });
          ...
          console.warn(
            "[TunManager] Service installation failed:",
            installResult.error,
          );
    ...
    try {
      ...
      if (!sameKernel) {
        ...
        console.log("[TunManager] Kernel update detected:", {
          source: {
            path: sourceKernelPath,
            size: sourceStat.size,
            mtime: sourceStat.mtime,
          },
          system: {
            path: systemPath,
            size: systemStat.size,
            mtime: systemStat.mtime,
          },
        });
  ...
  async function toggleTun(enabled) {
    try {
      if (enabled) {
        if (isWindows) {
          console.log(
            "[TunManager] Windows detected, checking permission for TUN toggle...",
          );
          ...
          if (!perm || !perm.success || !perm.hasPermission) {
            ...
            if (isInstalled === false) {
              return {
                success: false,
                error:
                  "TUN 服务未安装，请在 TUN 设置页面安装服务，或以管理员身份运行 Liberbox",
              };
            }
            if (isInstalled && isRunning === false) {
              return {
                success: false,
                error:
                  "TUN 服务未运行，请在 TUN 设置页面启动服务，或以管理员身份运行 Liberbox",
              };
            }
            if (!isAdmin && !isRunning) {
              return {
                success: false,
                error: "TUN 模式需要服务模式或以管理员身份运行 Liberbox",
              };
            }

            return {
              success: false,
              error: "TUN 模式缺少必要权限，请先完成授权",
            };
          }
        } else {
          console.log(
          ...
          if (isMac) {
            ...
            if (!perm || !perm.success || !perm.hasPermission) {
              ...
              const error = isKernelUpdated
                ...
                : "缺少必要权限，请先进行授权";
              console.warn(
                "[TunManager] macOS kernel permission check failed:",
                perm?.details || {},
              );
            ...
            if (!probe.ok) {
              console.warn(
                "[TunManager] Missing permissions, cannot enable TUN",
              );
      ...
      if (!updateUserSettingsRaw)
        return { success: false, error: "TUN 模式切换失败" };
      ...
      const baseTun = savedTun
        ? {
            ...
            mtu: savedTun.mtu,
            ...(isMac && savedTun.autoSetDNS !== undefined
              ? { "auto-set-dns": savedTun.autoSetDNS }
              : {}),
          }
        : {
            enable: enabled,
      ...
      const ok = await context.mihomoService?.restartMihomo?.(
        context.state.configFilePath,
      );
      if (!ok) {
        // rollback
      ...
      if (enabled && !isWindows) {
        ...
        if (isMac) {
          console.log(
            "[TunManager] Attempting to set system DNS for TUN mode...",
          );
          const dnsResult = await setSystemDns("198.18.0.1");
          if (dnsResult.success) {
            console.log(
              "[TunManager] ✓ System DNS set to 198.18.0.1 via service",
            );
          } else {
            console.log(
              "[TunManager] ℹ DNS not set via system (using TUN DNS hijacking):",
              dnsResult.reason,
            );
        ...
        if (isMac) {
          console.log(
            "[TunManager] Attempting to restore original system DNS...",
          );
          ...
          if (restoreResult.success) {
            ...
            console.log(
              "[TunManager] ℹ DNS not restored via service:",
    ...
    try {
      if (isMac) {
        ...
        if (fs.existsSync(systemKernelPath)) {
          ...
          if (
            sourceKernelPath &&
            fs.existsSync(sourceKernelPath) &&
            path.resolve(sourceKernelPath) !== path.resolve(systemKernelPath)
          ) {
            versionMatch = isSameKernelBinary(
              sourceKernelPath,
              systemKernelPath,
            );
          }
          ...
          if (systemProbe.ok && !versionMatch) {
            console.warn(
              "[TunManager] System kernel is authorized but outdated, re-authorization required",
            );
            return {
              success: true,
              hasPermission: false,
      ...
      if (isLinux) {
        ...
        return {
          success: true,
          hasPermission: ok,
          details: { path: kernelPath, stat: st },
        };
      }
      ...
      if (isWindows) {
        const elevationMode = getTunElevationMode();
        console.log(
          "[TunManager] Windows checking permission, mode:",
          elevationMode,
        );
        ...
        if (coreService) {
          try {
            ...
            console.warn(
              "[TunManager] Windows service status check failed:",
              e.message,
            );
          }
        }
        ...
        try {
          ...
          console.warn(
            "[TunManager] Windows task/admin status check failed:",
            e.message,
          );
        }
