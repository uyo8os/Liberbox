/**
 * Liberbox Helper Service IPC Client
...
const PIPE_NAME = "\\\\.\\pipe\\liberbox-helper-service";
const SECRET_SEED = "liberbox-helper-service-secret-key-v1";
const MESSAGE_EXPIRY_SECS = 30;
const SERVICE_NAME = "LiberboxHelperService";

// 派生密钥（优先读取动态密钥，回退到硬编码）
function loadSecretKey() {
  // 尝试读取动态密钥
  const pd = process.env.ProgramData || "C:\\ProgramData";
  const keyPath = path.join(pd, "Liberbox", "service-key");
  try {
...
function getHelperPath() {
  ...
  const possiblePaths = [
    ...
    path.join(process.resourcesPath || "", "liberbox-helper.exe"),
    path.join(process.resourcesPath || "", "liberbox-helper.exe"),
    // 开发环境路径
    path.join(app.getAppPath(), "tools", "liberbox-helper.exe"),
    path.join(app.getAppPath(), "tools", "liberbox-helper.exe"),
    path.join(__dirname, "..", "..", "tools", "liberbox-helper.exe"),
    path.join(__dirname, "..", "..", "tools", "liberbox-helper.exe"),
    path.join(__dirname, "..", "..", "native", "helper", "liberbox-helper.exe"),
    path.join(__dirname, "..", "..", "native", "helper", "liberbox-helper.exe"),
  ];
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code === 0) {
        resolve({
          success: true,
          message: stdout.trim() || "Service installed successfully",
        });
      } else {
        resolve({
          success: false,
          error: stderr.trim() || `Installation failed with exit code: ${code}`,
        });
      }
    });
...
async function uninstallService() {
  ...
  if (!deleted) {
    return {
      success: false,
      error: "服务删除未完成，请关闭服务管理器后重试或重启系统",
    };
  }
