...
start = (shadowsocks/custom/vmess/trojan/https/http/snell/socks5/socks5_tls/tuic/tuic_v5/wireguard/hysteria2/ssh/direct/reject) {
    return proxy;
}
...
custom = tag equals "custom" comma server:$[^,]+ comma port:$[0-9]+ comma method:$[^,]+ comma password:$[^,]+ comma obfs_uri:$[^\r\n]+ {
    proxy.type = "ss";
    proxy.server = server.trim();
    proxy.port = parseInt(port.trim());
    proxy.cipher = method.trim();
    proxy.password = password.trim();
    proxy.plugin = "obfs";
    $set(proxy, "plugin-opts.mode", "http");
    $set(proxy, "plugin-opts.host", obfs_uri.trim());
}
reject = tag equals "reject" {
    proxy.type = "reject";
}
