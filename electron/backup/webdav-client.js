...
class WebDAVClient {
   ...
   * @param {string} config.backupDirectory - 备份目录（默认"Liberbox"）
   * @param {string} config.fileName - 备份文件名（默认"liberbox_backup.zip"）
   */
  constructor(config) {
    this.config = config;
    this.client = null;
   ...
   */
  getBackupDir() {
    let dir = (this.config.backupDirectory || "Liberbox").trim() || "Liberbox";
    // 移除开头的斜杠（如果有）
    if (dir.startsWith("/")) {
      dir = dir.substring(1);
    }
  ...
  getBackupFilePath(fileName = null) {
    const dir = this.getBackupDir();
    const name = fileName || this.config.fileName || "liberbox_backup.zip";
    // 确保路径格式正确
    return `/${dir}/${name}`;
  }
  ...
  /**
   * 生成带时间戳的备份文件名
   * @returns {string} 带时间戳的文件名，如 liberbox_backup_20251026_143022.zip
   */
  generateTimestampedFileName() {
    ...
    return `liberbox_backup_${timestamp}.zip`;
  }
  ...
  async uploadBackup(localFilePath, onProgress = null, useTimestamp = true) {
    try {
      ...
      console.log(
        `[WebDAVClient] 文件大小: ${(totalBytes / 1024 / 1024).toFixed(2)} MB`,
      );

      // 生成文件名（带时间戳或固定名称）
      const fileName = useTimestamp
        ? this.generateTimestampedFileName()
        : this.config.fileName || "liberbox_backup.zip";
  ...
  async downloadBackup(localFilePath, onProgress = null, fileName = null) {
    try {
      ...
      // 验证下载的文件大小
      const downloadedSize = Buffer.isBuffer(fileBuffer)
        ? fileBuffer.length
        : fileBuffer.byteLength;
      console.log(
        `[WebDAVClient] 下载字节数: ${downloadedSize}, 预期: ${totalBytes}`,
      );

      if (downloadedSize !== totalBytes) {
        console.warn(
          `[WebDAVClient] 文件大小不匹配！下载: ${downloadedSize}, 预期: ${totalBytes}`,
        );
  ...
  async listBackups() {
    try {
      ...
      const backups = contents
        .filter(
          (item) => item.type === "file" && item.filename.endsWith(".zip"),
        )
        .map((item) => ({
          name: item.basename,
          size: item.size,
