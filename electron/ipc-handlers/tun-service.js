...
function registerTunServiceHandlers(deps) {
  ...
  ipcMain.handle("toggleTunMode", async (event, token, enabled) => {
    try {
      if (!verifyAuthToken(token)) {
        security?.logSecurityEvent?.("invalid-token", {
      ...
      console.log(
        "[IPC toggleTunMode] 操作完成，当前状态:",
        state.tunModeEnabled,
        "result:",
        res,
      );
      ...
      if (!res || !res.success) {
        return {
          success: false,
          error: res && res.error ? String(res.error) : "切换 TUN 模式失败",
        };
      }
  ...
  ipcMain.handle("getTunStatus", async () => {
    try {
      const persisted =
        typeof context.getTunModeEnabled === "function"
      ...
      if (isWindows) {
        ...
        if (isDev) {
          ...
          dialog.showMessageBoxSync(state.mainWindow, {
            ...
            message: "开发环境下无法自动重启",
            detail:
              "在开发环境下，请手动以管理员权限运行 npm run electron:dev。\n\n或者使用打包后的应用进行 TUN 模式测试。",
            buttons: ["我知道了"],
            defaultId: 0,
            noLink: true,
          });
        ...
        // 生成任务 XML
        const taskFilePath = path.join(taskDir, "liberbox-task.xml");
...
`;
        ...
        // 释放单实例锁
        if (
          typeof app.hasSingleInstanceLock === "function" &&
          app.hasSingleInstanceLock()
        ) {
          app.releaseSingleInstanceLock();
          console.log(
            "[TUN] Released single instance lock before elevated restart",
          );
        }
        ...
        child.on("exit", (code) => {
          ...
          if (!hasSuccessMarker && code === 0) {
            console.log(
              "[TUN] Success marker missing but exit code indicates success, proceeding to quit",
            );
          }
          ...
          if (hasSuccessMarker || code === 0) {
            ...
            console.log(
              "[TUN] Success marker not found and exit code is non-zero; user may have cancelled UAC or script failed",
            );
          }
        });
        ...
        console.log(
          "[TUN] PowerShell process spawned, waiting for completion...",
        );
  ...
  ipcMain.handle("check-core-permission", async () => {
    try {
      ...
      if (isWindows) {
        if (
          context.tunManager &&
          typeof context.tunManager.checkPermission === "function"
        ) {
  ...
  ipcMain.handle("get-tun-elevation-mode", async () => {
    try {
      if (context.tunManager && context.tunManager.getTunElevationMode) {
        return {
          success: true,
          mode: context.tunManager.getTunElevationMode(),
        };
      }
    ...
    try {
      if (context.tunManager && context.tunManager.setTunElevationMode) {
        ...
        if (mode === "service" && context.permissionManager) {
          try {
            const taskExists =
              await context.permissionManager.checkElevateTask();
            if (taskExists) {
      ...
      if (context.tunManager && context.tunManager.installService) {
        ...
        if (isWindows && result && result.needsAdmin) {
          return {
            success: false,
            error:
              "安装服务需要管理员权限，请先退出 Liberbox，然后右键以管理员身份运行后再尝试安装服务。",
          };
        }
      ...
      if (context.tunManager && context.tunManager.uninstallService) {
        ...
        if (isWindows && result && result.needsAdmin) {
          return {
            success: false,
            error:
              "卸载服务需要管理员权限，请先退出 Liberbox，然后右键以管理员身份运行后再尝试卸载服务。",
          };
        }
