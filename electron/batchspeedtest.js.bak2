...
async function saveSpeedtestReport(reportData, userDataPath) {
  try {
    ...
    // 复制logo到报告目录中，确保HTML可以正确引用
    const logoOutputPath = path.join(reportsDir, `logo_${reportId}.png`);
    const cornerLogoOutputPath = path.join(reportsDir, `corner_logo_${reportId}.png`);
    const logoInputPath = path.join(__dirname, '..', 'public', 'logo.png');
    
    try {
      // 复制logo文件到报告目录，一份用于左上角，一份用于右上角
      await fs.copyFile(logoInputPath, logoOutputPath);
      await fs.copyFile(logoInputPath, cornerLogoOutputPath);
      console.log(`[batchspeedtest] Logo已复制到: ${logoOutputPath}`);
    } catch (logoErr) {
      console.error(`[batchspeedtest] 复制logo文件失败:`, logoErr);
      // 继续执行，即使logo复制失败
    }
    ...
    const reportWithMeta = {
      ...
      logoPath: `logo_${reportId}.png`, // 添加logo路径信息
      cornerLogoPath: `corner_logo_${reportId}.png` // 添加右上角logo路径信息
    };
...
async function generateSpeedtestReportWithPuppeteer(reportData, userDataPath, saveFilePath = null) {
  try {
    ...
    // 只有当用户没有指定自定义保存路径时，才保存JSON用于历史记录
    // 这样避免每次导出都创建一个新的历史记录
    if (!saveFilePath) {
      // 同时保存原始JSON数据，便于后续加载
    ...
    try {
      ...
      return {
        ...
        jsonPath: saveFilePath ? null : path.join(reportsDir, `${reportId}.json`),
...
function generateReportHtml(reportData) {
  ...
  testResults.forEach((result, index) => {
    ...
    const getSpeedColor = (speed) => {
      ...
      if (speed < 5) return 'speed-level-1';
      if (speed < 10) return 'speed-level-2';
      if (speed < 20) return 'speed-level-3';
      if (speed < 50) return 'speed-level-4';
      if (speed < 100) return 'speed-level-5';
      return 'speed-level-6';
    };
    ...
    tableRows += `
      <tr class="${index % 2 === 0 ? 'row-even' : 'row-odd'}">
        ...
        <td class="text-left node-name" title="${result.name}">
        ...
        <td class="text-center">
          <div class="value-with-icon">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon-sm" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"></path><path d="m19 9-5 5-4-4-3 3"></path></svg>
        ...
        <td class="text-center speed-value">
          <div class="value-with-icon">
            <svg xmlns="http://www.w3.org/2000/svg" class="icon-sm" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 18 0 9 9 0 0 0-18 0"></path><path d="M12 7v5l2.5 2.5"></path></svg>
      ...
      <div class="report-header">
        <div>
          <div class="logo-container">
            <div class="logo-box">
              ${reportData.logoPath ? 
                `<img src="${reportData.logoPath}" alt="FlyClash Logo" class="logo-img" />` :
      ...
      <div class="report-info-tags">
        ...
        <div class="info-tag tag-included">
          <svg xmlns="http://www.w3.org/2000/svg" class="icon-sm mr-1" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <path d="M9 11V9a2 2 0 0 1 2-2v0a2 2 0 0 1 2 2v2"></path>
