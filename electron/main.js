...
// 新的 helper 服务是独立的 Go 程序，不再需要 service-worker 模式
{
  ...
  // 修改应用的 appName，确保保存在 Roaming 目录下的文件夹名为 liberbox
  app.name = "liberbox";
  ...
  const dbPath = path.join(configDir, "liberbox.db");
  ...
  if (!migrationManager.isMigrated()) {
    console.log(
      "[Startup] Detected unmigrated data, starting auto migration...",
    );
  ...
  if (process.platform === "win32") {
    app.commandLine.appendSwitch(
      "enable-features",
      "DirectWriteFontCache,PlatformHEVCDecoderSupport",
    );
    app.commandLine.appendSwitch(
      "disable-features",
  ...
  const kernelPathManager = createKernelPathManager({
    app,
    context,
    userDataPath,
  });
  ...
  // 注册流量历史处理器
  const {
    registerTrafficHistoryHandlers,
  } = require("./ipc-handlers/traffic-history");
  ...
  } = require("./ipc-handlers/loopback-ipc");
  registerLoopbackIpcHandlers();
  ...
  const { ensureUserSettingsFile, getUserSettings, updateUserSettingsRaw } =
    context;

  // 窗口背景效果管理
  const { createBackdropManager } = require("./window/backdrop");
  const backdropManager = createBackdropManager({
    state,
    dbManager,
    enableAcrylic,
    isWindows,
  ...
  function updateUserSettings(settings) {
    try {
      ...
      console.error(
        "[updateUserSettings] Error updating user settings:",
        error,
      );
      return false;
    }
  ...
  async function fetchConnectionsInfo() {
    try {
      if (!state.activeApiConfig) {
        console.error(
          "[Debug] Cannot get connection info: API config unavailable",
        );
        return;
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          const activeConnections = data.connections
            ? data.connections.filter((conn) => conn.isActive !== false).length
            : 0;
          ...
          console.error(
            `[Debug] Connection info request failed: status ${response.status}`,
          );
        }
      } catch (fetchError) {
  ...
  function handleMihomoProcessExit(code, exitedPid = null) {
    if (
      exitedPid &&
      state.mihomoProcess &&
      state.mihomoProcess.pid !== exitedPid
    ) {
      console.log(
        `[MihomoExit] Ignore stale exit event: pid=${exitedPid}, activePid=${state.mihomoProcess.pid}`,
      );
      return;
    }
    console.log(
      `Mihomo process exited, code: ${code}, pid: ${exitedPid ?? "unknown"}`,
    );
  ...
  } = require("./ipc-handlers/window-control");
  ...
  } = require("./ipc-handlers/kernel-path-ipc");
  registerKernelPathHandlers({
    state,
    context,
    isWindows,
  ...
  } = require("./ipc-handlers/core-manager-ipc");
  ...
  app.whenReady().then(async () => {
    ...
    } = require("./ipc-handlers/settings-ipc");
    registerSettingsIpcHandlers({
      state,
      dbManager,
      security,
    ...
    } = require("./ipc-handlers/appearance-ipc");
    registerAppearanceIpcHandlers({
      state,
      dbManager,
      isWindows,
    ...
    } = require("./ipc-handlers/proxy-node-ipc");
    registerProxyNodeIpcHandlers({
      state,
      context,
      fetchMihomoAPI,
    ...
    } = require("./ipc-handlers/tun-service");
    registerTunServiceHandlers({
      context,
      state,
      dbManager,
