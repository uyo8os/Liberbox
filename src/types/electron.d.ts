...
export interface ElectronAPI {
  ...
  selectKernelExecutable: () => Promise<{
    success: boolean;
    path?: string;
  ...
  coreGetInstalledCores: () => Promise<{
    success: boolean;
    cores?: InstalledCore[];
    error?: string;
  }>;
  coreCheckUpdate: (coreType: CoreType) => Promise<{
    success: boolean;
    hasUpdate?: boolean;
  ...
  coreDownloadCore: (coreType: CoreType) => Promise<{
    success: boolean;
    version?: string;
    path?: string;
    error?: string;
  ...
  ) => Promise<{ success: boolean; versions?: CoreVersion[]; error?: string }>;
  ...
  onCoreDownloadProgress: (
    callback: (data: CoreDownloadProgress) => void,
  ) => () => void;
  ...
  getAppearanceMode: () => Promise<{
    success: boolean;
    mode: string;
    error?: string;
  }>;
  supportsAdvancedBackdrop: () => Promise<{
    success: boolean;
    supported: boolean;
  }>;
  onAppearanceModeChanged?: (
    callback: (mode: "acrylic" | "dynamic" | "solid" | "custom") => void,
  ) => () => void;
  ...
  getCustomBackground: () => Promise<{
    success: boolean;
    config?: { imagePath: string; opacity: number; blur: number };
    error?: string;
  }>;
  clearCustomBackground: () => Promise<{ success: boolean; error?: string }>;
  onCustomBackgroundApply: (
    callback: (config: {
      imageData?: string;
      imagePath?: string;
      opacity: number;
  ...
  saveSubscriptionOrder: (
    orderList: Array<{ path: string; order: number }>,
  ) => Promise<{ success: boolean; error?: string }>;
  ...
  grantTunPermissions: () => Promise<{
    success: boolean;
    message?: string;
    error?: string;
    needRestart?: boolean;
  }>;
  checkCorePermission: () => Promise<{
    success: boolean;
    hasPermission: boolean;
  }>;
  revokeCorePermission: () => Promise<{ success: boolean; error?: string }>;
  getTunConfig: () => Promise<{
    success: boolean;
    config?: TunConfig;
    error?: string;
  }>;
  ...
  installTunService?: () => Promise<{
    success: boolean;
    message?: string;
    error?: string;
  }>;
  uninstallTunService?: () => Promise<{
    success: boolean;
    message?: string;
    error?: string;
  }>;
  startTunService?: () => Promise<{
    success: boolean;
    message?: string;
    error?: string;
  }>;
  stopTunService?: () => Promise<{
    success: boolean;
    message?: string;
    error?: string;
  }>;
  ...
  saveHostsConfig: (
    hosts: Array<{ domain: string; value: string }>,
    configPath?: string,
  ) => Promise<{ success: boolean; error?: string }>;
  getSnifferConfig: (
    configPath?: string,
  ...
  getProxyGroupsConfig: (
    configPath: string,
  ) => Promise<{ success: boolean; groups?: any[]; error?: string }>;
  ...
  getProvidersConfig: (configPath: string) => Promise<{
    success: boolean;
  ...
  getProxiesConfig: (
    configPath: string,
  ) => Promise<{ success: boolean; proxies?: any[]; error?: string }>;
  saveProxiesConfig: (
    proxies: any[],
    configPath: string,
  ) => Promise<{ success: boolean; error?: string }>;
  ...
  onAiProxyStreamChunk: (
    callback: (requestId: string, chunk: Uint8Array) => void,
  ) => () => void;
  onAiProxyStreamEnd: (callback: (requestId: string) => void) => () => void;
  onAiProxyStreamError: (
    callback: (requestId: string, error: string) => void,
  ) => () => void;
  ...
  converter?: {
    ...
    addToConfig: (params: { name: string; url: string }) => Promise<{
      success: boolean;
      id?: string;
      filePath?: string;
      error?: string;
