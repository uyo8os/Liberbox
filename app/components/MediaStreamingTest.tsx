...
const MediaStreamingTest: React.FC<MediaStreamingTestProps> = ({
  currentNode,
  onTestComplete,
}) => {
  const [services, setServices] =
    useState<StreamingService[]>(DEFAULT_SERVICES);
  const [isTestRunning, setIsTestRunning] = useState(false);
  const [testProgress, setTestProgress] = useState(0);
  const [testStartTime, setTestStartTime] = useState<Date | null>(null);
  const [testEndTime, setTestEndTime] = useState<Date | null>(null);
  const [selectedServiceLog, setSelectedServiceLog] = useState<string | null>(
    null,
  );
  ...
  const startTest = async () => {
    ...
    console.log(
      `开始检测${activeTab === "全部" ? "所有" : activeTab}服务(共${testQueue.length}个)，当前节点:`,
      currentNode,
    );
    console.log(
      "是否存在API:",
      window.electronAPI &&
        typeof window.electronAPI.testMediaStreaming === "function"
        ? "是"
        : "否",
    );
    ...
    for (let index = 0; index < testQueue.length; index++) {
      ...
      try {
        ...
        if (
          ...
          try {
            console.log(
              `尝试检测 ${service.name}，URL: ${service.checkUrl || "未指定"}`,
            );

            // 特别记录特定服务的调用
            if (
              service.name === "AbemaTV" ||
              service.name === "myTVSuper" ||
              service.name === "Bilibili港澳台" ||
              service.name === "Bilibili台湾"
            ) {
              console.log(
                `特殊服务检测: ${service.name}，确保名称匹配后端case语句`,
              );
            }

            result = await window.electronAPI.testMediaStreaming(
              service.name,
              service.checkUrl,
            );
        ...
        setServices((prev) => {
          ...
          if (serviceIndex !== -1) {
            ...
            if (result.logPath) {
              // 假设日志路径就是日志内容
              newServices[serviceIndex].logContent =
  ...
  const getStatusInfo = (
    ...
    const getNetflixUnlockType = (
      service: StreamingService,
    ): "full" | "non-original" | "original-only" | "none" => {
    ...
    if (
      ...
      if (unlockType === "full") {
        return {
          ...
          text: getDetailedStatusText(service),
        };
      } else {
        return {
    ...
    switch (status) {
      ...
      default:
        return {
          color: "text-gray-400 bg-gray-50 dark:bg-gray-900/20",
          icon: <AlertTriangle className="w-4 h-4" />,
          text: getDetailedStatusText(service),
        };
    }
  ...
  const generateReportImage = async () => {
    ...
    try {
      // 预先准备报告数据
      const successCount = services.filter(
        (s) => s.status === "success",
      ).length;
      const partialCount = services.filter(
        (s) => s.status === "partial",
      ).length;
      const errorCount = services.filter((s) => s.status === "error").length;

      // 获取所有需要显示的服务
      const servicesForReport = services.filter(
      ...
      // 构建服务卡片HTML
      const serviceCardsHtml = servicesForReport
      ...
      container.style.cssText =
        "position:fixed;left:-9999px;top:0;width:720px;background:white;";
        ...
        <div style="padding:24px;font-family:system-ui,-apple-system,sans-serif;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;">
            <div style="display:flex;align-items:center;">
              <img src="/logo.png" alt="Liberbox Logo" style="width:44px;height:44px;margin-right:12px;" />
              <div>
                <h1 style="margin:0;font-size:22px;font-weight:600;color:#1f2937;">Liberbox</h1>
  ...
  const downloadReportImage = () => {
    ...
    link.download = `Liberbox流媒体测试报告_${new Date().toISOString().split("T")[0]}.png`;
      ...
      <div className="flex flex-col gap-1 mb-2 flex-shrink-0">
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center">
            ...
            <Badge
              variant="outline"
              className="text-xs py-0 h-5 bg-gray-50 dark:bg-gray-800"
            >
              {currentNode || "未选择"}
            </Badge>
          </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <p className="text-sm font-medium mr-2">{service.name}</p>
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Badge
                              variant="outline"
                              className={cn(
                                "flex items-center space-x-1 text-xs py-0.5",
                                color,
                              )}
                            >
                              {icon}
                            ...
                            {service.ipInfo && (
                              <div className="mt-1 border-t pt-1 border-gray-200 dark:border-gray-700 text-xs">
                                ...
                                {service.ipInfo.country && (
                                  <p>
                                    位置: {service.ipInfo.country}{" "}
                                    {service.ipInfo.region}{" "}
                                    {service.ipInfo.city}
                                  </p>
                                )}
                                {service.ipInfo.org && (
